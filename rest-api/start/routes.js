...
Route.group(()=>{
  ...
  Route.get('proyectos/:id/tareas','ProyectoController.index').middleware('auth');
  Route.post('proyectos/:id/tareas','TareaController.create').middleware('auth');
  Route.patch('tareas/:id','ProyectoController.update').middleware('auth');
  Route.delete('tareas/:id','ProyectoController.destroy').middleware('auth');
}).prefix('api/v1');
